export const idlFactory = ({ IDL }) => {
  ...
  const Result_8 = IDL.Variant({
    'ok' : IDL.Vec(IDL.Tuple(RecordId, CandidBlob)),
    'err' : IDL.Text,
  });
  ...
  const Result_5 = IDL.Variant({ 'ok' : RecordId, 'err' : IDL.Text });
  CandidType.fill(
    IDL.Variant({
      'Int' : IDL.Null,
  ...
  const CanisterDB = IDL.Service({
    ...
    'zendb_collection_create_index' : IDL.Func(
