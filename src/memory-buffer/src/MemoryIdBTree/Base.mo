...
module {
    ...
    public func _new_with_options(order : ?Nat, opt_cache_size: ?Nat, is_set : Bool) : MemoryIdBTree {
        let cache_size = Option.get(opt_cache_size, CACHE_LIMIT);
        let btree_map : MemoryIdBTree = {
            is_set;
    ...
    public func new_set(order : ?Nat, cache_size: ?Nat) : MemoryIdBTree {
        _new_with_options(order, cache_size, true);
    };

    public func new(order : ?Nat) : MemoryIdBTree {
    ...
    func init_region_header(btree : MemoryIdBTree) {
    ...
    public func size(btree : MemoryIdBTree) : Nat {
        btree.count;
    };

    public func fromVersioned(btree: VersionedMemoryIdBTree) : MemoryIdBTree {
        Migrations.getCurrentVersion(btree);
    };

    public func toVersioned(btree: MemoryIdBTree) : VersionedMemoryIdBTree {
        #v0(btree);
    };

    public func bytes(btree : MemoryIdBTree) : Nat {
        MemoryRegion.allocated(btree.blobs);
    };

    public func metadataBytes(btree : MemoryIdBTree) : Nat {
        MemoryRegion.allocated(btree.metadata);
    };

    func update_root(btree : MemoryIdBTree, new_root : Address) {
        btree.root := new_root;
        MemoryRegion.storeNat64(btree.metadata, MC.ROOT_ADDRESS, Nat64.fromNat(new_root));
    };

    func update_count(btree : MemoryIdBTree, new_count : Nat) {
        btree.count := new_count;
        MemoryRegion.storeNat64(btree.metadata, MC.COUNT_ADDRESS, Nat64.fromNat(new_count));
    };

    func update_branch_count(btree : MemoryIdBTree, new_count : Nat) {
        btree.branch_count := new_count;
        MemoryRegion.storeNat64(btree.metadata, MC.BRANCH_COUNT, Nat64.fromNat(new_count));
    };

    func update_leaf_count(btree : MemoryIdBTree, new_count : Nat) {
        btree.leaf_count := new_count;
        MemoryRegion.storeNat64(btree.metadata, MC.LEAF_COUNT, Nat64.fromNat(new_count));
    };

    func inc_subtree_size(btree : MemoryIdBTree, branch_address : Nat, _child_index : Nat) {
    ...
    public func insert<K, V>(btree : MemoryIdBTree, btree_utils : BTreeUtils<K, V>, key : K, value : V) : ?V {
        ...
        if (count < btree.order) {
            ...
            Leaf.insert(btree, leaf_address, elem_index, key_val_id);
    ...
    public func reference<K, V>(btree : MemoryIdBTree, btree_utils : BTreeUtils<K, V>, id: UniqueId){
    ...
    public func getRefCount<K, V>(btree : MemoryIdBTree, btree_utils : BTreeUtils<K, V>, id: UniqueId) : ?Nat {
        if (not MemoryBlock.id_exists(btree, id)) return null;
        ?MemoryBlock.get_ref_count(btree, id);
    };

    public func lookup<K, V>(btree : MemoryIdBTree, btree_utils : BTreeUtils<K, V>, id: UniqueId) : ?(K, V) {
        if (not MemoryBlock.id_exists(btree, id)) return null;
    ...
    public func lookupKey<K, V>(btree : MemoryIdBTree, btree_utils : BTreeUtils<K, V>, id: UniqueId) : ?K {
        if (not MemoryBlock.id_exists(btree, id)) return null;
    ...
    public func lookupVal<K, V>(btree : MemoryIdBTree, btree_utils : BTreeUtils<K, V>, id: UniqueId) : ?V {
        if (not MemoryBlock.id_exists(btree, id)) return null;
    ...
    public func getId<K, V>(btree : MemoryIdBTree, btree_utils : BTreeUtils<K, V>, key: K) : ?UniqueId {
        let key_blob = btree_utils.key.to_blob(key);
    ...
    public func nextId<K, V>(btree : MemoryIdBTree) : UniqueId {
    ...
    public func toEntries<K, V>(btree: MemoryIdBTree, btree_utils: BTreeUtils<K, V>) : [(K, V)] {
        Utils.sized_iter_to_array<(K, V)>(entries(btree, btree_utils), btree.count);
    };

    public func toArray<K, V>(btree: MemoryIdBTree, btree_utils: BTreeUtils<K, V>) : [(K, V)] {
        Utils.sized_iter_to_array<(K, V)>(entries(btree, btree_utils), btree.count);
    };
    ...
    public func toKeys<K, V>(btree: MemoryIdBTree, btree_utils: BTreeUtils<K, V>) : [K] {
    ...
    public func toVals<K, V>(btree: MemoryIdBTree, btree_utils: BTreeUtils<K, V>) : [V] {
        Utils.sized_iter_to_array<V>(vals(btree, btree_utils), btree.count);
    };

    public func leafNodes<K, V>(btree : MemoryIdBTree, btree_utils: BTreeUtils<K, V>) : RevIter<[?(K, V)]> {
        Methods.leaf_nodes(btree, btree_utils);
    };

    public func toLeafNodes<K, V>(btree: MemoryIdBTree, btree_utils: BTreeUtils<K, V>) : [[?(K, V)]] {
        Utils.sized_iter_to_array<[?(K, V)]>(leafNodes<K, V>(btree, btree_utils), btree.leaf_count);
    };

    public func toNodeKeys<K, V>(btree: MemoryIdBTree, btree_utils: BTreeUtils<K, V>) : [[(Nat, [?K])]] {
        Methods.node_keys(btree, btree_utils);
    };

    public func get<K, V>(btree : MemoryIdBTree, btree_utils : BTreeUtils<K, V>, key : K) : ?V {
        let key_blob = btree_utils.key.to_blob(key);
    ...
    public func getMin<K, V>(btree : MemoryIdBTree, btree_utils : BTreeUtils<K, V>) : ?(K, V) {
    ...
    public func getMax<K, V>(btree : MemoryIdBTree, btree_utils : BTreeUtils<K, V>) : ?(K, V) {
    ...
    public func clear(btree : MemoryIdBTree) {
        // clear cache
    ...
    func decrement_subtree_size(btree : MemoryIdBTree, branch_address : Nat, _child_index : Nat) {
        let subtree_size = Branch.get_subtree_size(btree, branch_address);
        Branch.update_subtree_size(btree, branch_address, subtree_size - 1);
    };

    public func remove<K, V>(btree : MemoryIdBTree, btree_utils : BTreeUtils<K, V>, key : K) : ?V {
        let key_blob = btree_utils.key.to_blob(key);
    ...
    public func removeMin<K, V>(btree : MemoryIdBTree, btree_utils : BTreeUtils<K, V>) : ?(K, V) {
        let ?min = getMin<K, V>(btree, btree_utils) else return null;
        ignore remove<K, V>(btree, btree_utils, min.0);
        ?min
    };

    public func removeMax<K, V>(btree : MemoryIdBTree, btree_utils : BTreeUtils<K, V>) : ?(K, V) {
        let ?max = getMax<K, V>(btree, btree_utils) else return null;
        ignore remove<K, V>(btree, btree_utils, max.0);
        ?max
    };

    public func fromArray<K, V>(btree_utils: BTreeUtils<K, V>, arr : [(K, V)], order : ?Nat) : MemoryIdBTree {
        fromEntries(btree_utils, arr.vals(), order);
    };

    public func fromEntries<K, V>(btree_utils: BTreeUtils<K, V>, entries : Iter<(K, V)>, order : ?Nat) : MemoryIdBTree {
        let btree = new(order);
    ...
    public func getCeiling<K, V>(btree : MemoryIdBTree, btree_utils : BTreeUtils<K, V>, key : K) : ?(K, V) {
        let key_blob = btree_utils.key.to_blob(key);
    ...
    public func getFloor<K, V>(btree : MemoryIdBTree, btree_utils : BTreeUtils<K, V>, key : K) : ?(K, V) {
        let key_blob = btree_utils.key.to_blob(key);
        let leaf_address = Methods.get_leaf_address<K, V>(btree, btree_utils, key, ?key_blob);
    ...
    public func getFromIndex<K, V>(btree : MemoryIdBTree, btree_utils : BTreeUtils<K, V>, index : Nat) : (K, V) {
    ...
    public func getIndex<K, V>(btree : MemoryIdBTree, btree_utils : BTreeUtils<K, V>, key : K) : Nat {
        let key_blob = btree_utils.key.to_blob(key);
    ...
    public func range<K, V>(btree: MemoryIdBTree, btree_utils: BTreeUtils<K, V>, start:Nat, end:Nat): RevIter<(K, V)>{
    ...
    public func scan<K, V>(btree: MemoryIdBTree, btree_utils: BTreeUtils<K, V>, start:?K, end:?K): RevIter<(K, V)>{
