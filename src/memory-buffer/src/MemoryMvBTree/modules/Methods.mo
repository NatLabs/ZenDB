...
module {
    ...
    public func get_leaf_address<K, V>(btree : MemoryMvBTree, btree_utils : BTreeUtils<K, V>, key : K, _opt_key_blob : ?Blob) : Nat {
    ...
    public func get_leaf_address_and_update_path<K, V>(btree : MemoryMvBTree, btree_utils : BTreeUtils<K, V>, key : K, _opt_key_blob : ?Blob, update : (MemoryMvBTree, Nat, Nat) -> ()) : Nat {
        var curr_address = btree.root;
        var opt_key_blob : ?Blob = _opt_key_blob;
    ...
    public func get_min_leaf_address(btree : MemoryMvBTree) : Nat {
        var curr = btree.root;
    ...
    public func get_max_leaf_address(btree : MemoryMvBTree) : Nat {
        var curr = btree.root;
    ...
    public func update_leaf_to_root(btree : MemoryMvBTree, leaf_address : Nat, update : (MemoryMvBTree, Nat, Nat) -> ()) {
    ...
    public func update_branch_to_root(btree : MemoryMvBTree, branch_address : Nat, update : (MemoryMvBTree, Nat, Nat) -> ()) {
    ...
    public func get_leaf_node_and_index<K, V>(btree : MemoryMvBTree, btree_utils : BTreeUtils<K, V>, key : Blob) : (Address, Nat) {
    ...
    public func get_leaf_node_by_index<K, V>(btree : MemoryMvBTree, rank : Nat) : (Address, Nat) {
    ...
    public func new_internal_id_iterator(
        btree : MemoryMvBTree,
        start_leaf : Nat,
        start_index : Nat,
        end_leaf : Nat,
        ...
        func next() : ?Nat {
            ...
            let opt_kv = Leaf.get_kv_id(btree, start, i);
        ...
        func nextFromEnd() : ?Nat {
            ...
            let opt_kv = Leaf.get_kv_id(btree, end, j - 1);
    ...
    // public func new_blobs_iterator_v2(
    //     btree : MemoryMvBTree,
    //     start_leaf : Nat,
    //     start_index : Nat,
    //     end_leaf : Nat,
    ...
    //     RevIter.flatten<(Blob, Blob)>(
    //         RevIter.map<Nat, RevIter<(Blob, Blob)>>(
    //             new_internal_id_iterator(btree, start_leaf, start_index, end_leaf, end_index),
    //             func(id : Nat) : RevIter<(Blob, Blob)> {
    //                 let key = MemoryBlock.get_key_blob(btree, id);
    //                 let vals = MemoryBlock.get_val_blobs(btree, id);

    //                 let polyfill_to_rev_iter = RevIter.new(
    //                     func() : ?Blob { vals.next() },
    //                     func() : ?Blob { vals.next() },
    //                 );

    //                 RevIter.map<Blob, (Blob, Blob)>(
    //                     polyfill_to_rev_iter,
    ...
    public func key_val_blobs(btree : MemoryMvBTree) : RevIter<(Blob, Blob)> {
    ...
    public func leaf_addresses(btree : MemoryMvBTree) : RevIter<Nat> {
        let min_leaf = get_min_leaf_address(btree);
        let max_leaf = get_max_leaf_address(btree);
    ...
    public func leaf_nodes<K, V>(btree : MemoryMvBTree, btree_utils : BTreeUtils<K, V>) : RevIter<[?(K, V)]> {
        let min_leaf = get_min_leaf_address(btree);
        let max_leaf = get_max_leaf_address(btree);
    ...
    public func node_keys<K, V>(btree : MemoryMvBTree, btree_utils : BTreeUtils<K, V>) : [[(Nat, [?K])]] {
    ...
    public func validate_memory(btree : MemoryMvBTree, btree_utils : BTreeUtils<Nat, Nat>) : Bool {
