/// A persistent BTree Index that stores key-value pairs in stable memory
/// The key can be any type but the values are stored as Nat to save space as the values in an
/// index are usually unique ids that are used to reference values located in a separate data store.
...
module {
    ...
    public func _new_with_options(order : ?Nat, opt_cache_size: ?Nat, is_set : Bool) : MemoryBTreeIndex {
        let cache_size = Option.get(opt_cache_size, CACHE_LIMIT);
        let btree_map : MemoryBTreeIndex = {
            is_set;
    ...
    public func new_set(order : ?Nat, cache_size: ?Nat) : MemoryBTreeIndex {
        _new_with_options(order, cache_size, true);
    };

    public func new(order : ?Nat) : MemoryBTreeIndex {
    ...
    func init_region_header(btree : MemoryBTreeIndex) {
    ...
    public func size(btree : MemoryBTreeIndex) : Nat {
        btree.count;
    };

    public func fromVersioned(btree: VersionedMemoryBTreeIndex) : MemoryBTreeIndex {
        Migrations.getCurrentVersion(btree);
    };

    public func toVersioned(btree: MemoryBTreeIndex) : VersionedMemoryBTreeIndex {
        #v0(btree);
    };

    public func bytes(btree : MemoryBTreeIndex) : Nat {
        MemoryRegion.allocated(btree.blobs);
    };

    public func metadataBytes(btree : MemoryBTreeIndex) : Nat {
        MemoryRegion.allocated(btree.metadata);
    };

    func update_root(btree : MemoryBTreeIndex, new_root : Address) {
        btree.root := new_root;
        MemoryRegion.storeNat64(btree.metadata, MC.ROOT_ADDRESS, Nat64.fromNat(new_root));
    };

    func update_count(btree : MemoryBTreeIndex, new_count : Nat) {
        btree.count := new_count;
        MemoryRegion.storeNat64(btree.metadata, MC.COUNT_ADDRESS, Nat64.fromNat(new_count));
    };

    func update_branch_count(btree : MemoryBTreeIndex, new_count : Nat) {
        btree.branch_count := new_count;
        MemoryRegion.storeNat64(btree.metadata, MC.BRANCH_COUNT, Nat64.fromNat(new_count));
    };

    func update_leaf_count(btree : MemoryBTreeIndex, new_count : Nat) {
        btree.leaf_count := new_count;
        MemoryRegion.storeNat64(btree.metadata, MC.LEAF_COUNT, Nat64.fromNat(new_count));
    };

    func inc_subtree_size(btree : MemoryBTreeIndex, branch_address : Nat, _child_index : Nat) {
    ...
    public func insert<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>, key : K, value : Nat) : ?Nat {
        let key_blob = btree_utils.blobify.to_blob(key);
        ...
        let int_index = switch (btree_utils.cmp) {
            case (#cmp(cmp)) Leaf.binary_search<K>(btree, btree_utils, leaf_address, cmp, key, count);
            case (#blob_cmp(cmp)) {
        ...
        if (int_index >= 0) {
            ...
            let prev_val = MemoryBlock.get_val(btree, id);
            MemoryBlock.replace_val(btree, id, value);
        ...
        let key_val_id = MemoryBlock.store(btree, key_blob, value);
        // Debug.print("key_val_id: " # debug_show key_val_id);
        // Debug.print("kv_blobs: " # debug_show( MemoryBlock.get_key_blob(btree, key_val_id), MemoryBlock.get_val(btree, key_val_id)));
        // Debug.print("actual kv_blobs: " # debug_show (key_blob, value));
    ...
    public func entries<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>) : RevIter<(K, Nat)> {
        Methods.entries(btree, btree_utils);
    };

    public func toEntries<K>(btree: MemoryBTreeIndex, btree_utils: IndexUtils<K>) : [(K, Nat)] {
        Utils.sized_iter_to_array<(K, Nat)>(entries(btree, btree_utils), btree.count);
    };

    public func toArray<K>(btree: MemoryBTreeIndex, btree_utils: IndexUtils<K>) : [(K, Nat)] {
        Utils.sized_iter_to_array<(K, Nat)>(entries(btree, btree_utils), btree.count);
    };

    public func keys<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>) : RevIter<K> {
        Methods.keys(btree, btree_utils);
    };

    public func toKeys<K>(btree: MemoryBTreeIndex, btree_utils: IndexUtils<K>) : [K] {
        Utils.sized_iter_to_array<K>(keys(btree, btree_utils), btree.count);
    };

    public func vals<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>) : RevIter<Nat> {
        Methods.vals(btree, btree_utils);
    };

    public func toVals<K>(btree: MemoryBTreeIndex, btree_utils: IndexUtils<K>) : [Nat] {
        Utils.sized_iter_to_array<Nat>(vals(btree, btree_utils), btree.count);
    };

    public func leafNodes<K>(btree : MemoryBTreeIndex, btree_utils: IndexUtils<K>) : RevIter<[?(K, Nat)]> {
        Methods.leaf_nodes(btree, btree_utils);
    };

    public func toLeafNodes<K>(btree: MemoryBTreeIndex, btree_utils: IndexUtils<K>) : [[?(K, Nat)]] {
        Utils.sized_iter_to_array<[?(K, Nat)]>(leafNodes<K>(btree, btree_utils), btree.leaf_count);
    };

    public func toNodeKeys<K>(btree: MemoryBTreeIndex, btree_utils: IndexUtils<K>) : [[(Nat, [?K])]] {
        Methods.node_keys(btree, btree_utils);
    };

    public func get<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>, key : K) : ?Nat {
        let key_blob = btree_utils.blobify.to_blob(key);
        ...
        let ?value = Leaf.get_val(btree, leaf_address, elem_index) else Debug.trap("get: accessed a null value");
        ?value;
    };

    public func getMin<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>) : ?(K, Nat) {
        ...
        let ?value = Leaf.get_val(btree, leaf_address, 0) else Debug.trap("getMin: accessed a null value");

        let key = btree_utils.blobify.from_blob(key_blob);
        ?(key, value);
    };

    public func getMax<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>) : ?(K, Nat) {
        ...
        let ?value = Leaf.get_val(btree, leaf_address, count - 1 : Nat) else Debug.trap("getMax: accessed a null value");
    ...
    public func clear(btree : MemoryBTreeIndex) {
        // clear cache
        ...
        clear_region_after_header(btree.metadata, everything_after_leaf);

        clear_region_after_header(btree.blocks, BLOBS_REGION_HEADER_SIZE);

        // Debug.print("blobs size: " # debug_show MemoryRegion.size(btree.blobs));
        clear_region_after_header(btree.blobs, BLOBS_REGION_HEADER_SIZE);
        
    };

    func decrement_subtree_size(btree : MemoryBTreeIndex, branch_address : Nat, _child_index : Nat) {
        let subtree_size = Branch.get_subtree_size(btree, branch_address);
        Branch.update_subtree_size(btree, branch_address, subtree_size - 1);
    };

    public func remove<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>, key : K) : ?Nat {
        let key_blob = btree_utils.blobify.to_blob(key);
        ...
        let prev_val = MemoryBlock.get_val(btree, prev_kv_id);
        ...
        func set_only_child_to_root(parent: Address) : ?Nat {
    ...
    public func removeMin<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>) : ?(K, Nat) {
        let ?min = getMin<K>(btree, btree_utils) else return null;
        ignore remove<K>(btree, btree_utils, min.0);
        ?min
    };

    public func removeMax<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>) : ?(K, Nat) {
        let ?max = getMax<K>(btree, btree_utils) else return null;
        ignore remove<K>(btree, btree_utils, max.0);
        ?max
    };

    public func fromArray<K>(btree_utils: IndexUtils<K>, arr : [(K, Nat)], order : ?Nat) : MemoryBTreeIndex {
        fromEntries(btree_utils, arr.vals(), order);
    };

    public func fromEntries<K>(btree_utils: IndexUtils<K>, entries : Iter<(K, Nat)>, order : ?Nat) : MemoryBTreeIndex {
        let btree = new(order);
    ...
    public func getCeiling<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>, key : K) : ?(K, Nat) {
        let key_blob = btree_utils.blobify.to_blob(key);
        let leaf_address = Methods.get_leaf_address<K>(btree, btree_utils, key, ?key_blob);

        let i = switch(btree_utils.cmp){
            case(#cmp(cmp)) Leaf.binary_search<K>(btree, btree_utils, leaf_address, cmp, key, Leaf.get_count(btree, leaf_address));
            case(#blob_cmp(cmp)) {
        ...
        if (i >= 0) {
            ...
            return ?Methods.deserialize_kv_blobs<K>(btree_utils, k, v)
        };
    ...
    public func getFloor<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>, key : K) : ?(K, Nat) {
        let key_blob = btree_utils.blobify.to_blob(key);
        let leaf_address = Methods.get_leaf_address<K>(btree, btree_utils, key, ?key_blob);
        ...
        if (i >= 0) {
            ...
            return ?Methods.deserialize_kv_blobs<K>(btree_utils, kv_blobs.0, kv_blobs.1);
        };
    ...
    public func getFromIndex<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>, index : Nat) : (K, Nat) {
    ...
    public func getIndex<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>, key : K) : Nat {
        let key_blob = btree_utils.blobify.to_blob(key);
    ...
    public func range<K>(btree: MemoryBTreeIndex, btree_utils: IndexUtils<K>, start:Nat, end:Nat): RevIter<(K, Nat)>{
        ...
        RevIter.map<(Blob, Nat), (K, Nat)>(
            Methods.new_blobs_iterator(btree, start_node, start_index, end_node, end_index),
            func((key_blob, value) : (Blob, Nat)) : (K, Nat) {
                Methods.deserialize_kv_blobs<K>(btree_utils, key_blob, value);
            },
        );
    };

    public func scan<K>(btree: MemoryBTreeIndex, btree_utils: IndexUtils<K>, start:?K, end:?K): RevIter<(K, Nat)>{
        ...
        let start_index = switch(start){
            case(?key) switch(btree_utils.cmp){
                case(#cmp(cmp)) Leaf.binary_search<K>(btree, btree_utils, start_address, cmp, key, Leaf.get_count(btree, start_address));
                case(#blob_cmp(cmp)) {
                    let key_blob = btree_utils.blobify.to_blob(key);
        ...
        let end_index = switch(end){
            case(?key) switch(btree_utils.cmp){
                case(#cmp(cmp)) Leaf.binary_search<K>(btree, btree_utils, end_address, cmp, key, Leaf.get_count(btree, end_address));
                case(#blob_cmp(cmp)) {
                    let key_blob = btree_utils.blobify.to_blob(key);
