...
module {
    ...
    public func get_leaf_address<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>, key : K, _opt_key_blob : ?Blob) : Nat {
            ...
            switch (Branch.get_node_type(btree, curr_address)) {
                ...
                case (#branch) {
                    ...
                    let int_index = switch (btree_utils.cmp) {
                        case (#cmp(cmp)) Branch.binary_search<K>(btree, btree_utils, curr_address, cmp, key, count - 1);
                        case (#blob_cmp(cmp)) {
    ...
    public func get_leaf_address_and_update_path<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>, key : K, _opt_key_blob : ?Blob, update : (MemoryBTreeIndex, Nat, Nat) -> ()) : Nat {
        var curr_address = btree.root;
        var opt_key_blob : ?Blob = _opt_key_blob;
    ...
    public func get_min_leaf_address(btree : MemoryBTreeIndex) : Nat {
        var curr = btree.root;
    ...
    public func get_max_leaf_address(btree : MemoryBTreeIndex) : Nat {
        var curr = btree.root;
    ...
    public func update_leaf_to_root(btree : MemoryBTreeIndex, leaf_address : Nat, update : (MemoryBTreeIndex, Nat, Nat) -> ()) {
    ...
    public func update_branch_to_root(btree : MemoryBTreeIndex, branch_address : Nat, update : (MemoryBTreeIndex, Nat, Nat) -> ()) {
    ...
    public func get_leaf_node_and_index<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>, key : Blob) : (Address, Nat) {
                ...
                switch (Branch.get_node_type(btree, child)) {
                    case (#branch(_)) {
                        ...
                        switch(btree_utils.cmp) {
                            case (#cmp(cmp)) {
                                let ds_key = btree_utils.blobify.from_blob(key);
                                let ds_search_key = btree_utils.blobify.from_blob(search_key);
    ...
    public func get_leaf_node_by_index<K>(btree : MemoryBTreeIndex, rank : Nat) : (Address, Nat) {
    ...
    public func key_val_blobs(btree : MemoryBTreeIndex) : RevIter<(Blob, Nat)> {
    ...
    public func deserialize_kv_blobs<K>(btree_utils : IndexUtils<K>, key_blob : Blob, value : Nat) : (K, Nat) {
    ...
    public func entries<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>) : RevIter<(K, Nat)> {
        RevIter.map<(Blob, Nat), (K, Nat)>(
            key_val_blobs(btree),
            func((key_blob, val_blob) : (Blob, Nat)) : (K, Nat) {
    ...
    public func keys<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>) : RevIter<(K)> {
        RevIter.map<(Blob, Nat), (K)>(
            key_val_blobs(btree),
            func((key_blob, _) : (Blob, Nat)) : (K) {
                let key = btree_utils.blobify.from_blob(key_blob);
                key;
            },
        );
    ...
    public func vals<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>) : RevIter<Nat> {
        RevIter.map<(Blob, Nat), Nat>(
            key_val_blobs(btree),
            func((_, value) : (Blob, Nat)) : Nat {
                value;
            },
        );
    };
    ...
    public func leaf_addresses(btree : MemoryBTreeIndex) : RevIter<Nat> {
        let min_leaf = get_min_leaf_address(btree);
        let max_leaf = get_max_leaf_address(btree);
    ...
    public func leaf_nodes<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>) : RevIter<[?(K, Nat)]> {
        let min_leaf = get_min_leaf_address(btree);
        let max_leaf = get_max_leaf_address(btree);

        RevIter.map<Nat, [?(K, Nat)]>(
            new_leaf_address_iterator(btree, min_leaf, max_leaf),
            func(leaf_address : Nat) : [?(K, Nat)] {
                
                let count = Leaf.get_count(btree, leaf_address);
                Array.tabulate<?(K, Nat)>(
                    btree.order,
                    func(i : Nat) : ?(K, Nat) {
                        ...
                        ?(btree_utils.blobify.from_blob(key), val);
                    },
                );
            },
        );
    ...
    public func node_keys<K>(btree : MemoryBTreeIndex, btree_utils : IndexUtils<K>) : [[(Nat, [?K])]] {
    ...
    public func validate_memory(btree : MemoryBTreeIndex, btree_utils : IndexUtils<Nat>) : Bool {
                    ...
                    while (i < count) {
                        ...
                        switch (prev_key) {
                            ...
                            case (?prev) {
                                switch (btree_utils.cmp) {
                                    case (#cmp(cmp)) {
                                        let _prev = btree_utils.blobify.from_blob(prev);
                                        let _key = btree_utils.blobify.from_blob(key);
