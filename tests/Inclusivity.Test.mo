...
func inclusivity_tests(zendb : ZenDB.Database) {
    ...
    suite(
        ...
        func() {
            ...
            //         // get_next_value() will append a 1 to the blob
            //         let result = composite_collection.search(
            ...
            //         let a = Orchid.blobify.to_blob([#Text("X"), #Blob(Blob.fromArray([12, 32, 45]))]);
            //         let b = Orchid.blobify.to_blob([#Text("X"), #Blob(Blob.fromArray([12, 32, 45, 0]))]);

            //         Debug.print(debug_show a);
            //         Debug.print(debug_show b);
            //         Debug.print(debug_show (a == b, a < b, a > b));
...
suite(
    ...
    func() {
        suite(
            "Stable Memory",
            func() {
                // Create a stable store for testing
        ...
        suite(
            "Heap memory",
            func() {
                // Create a heap store for testing
                let zendb_hstore = ZenDB.newStableStore(
                    ?{
                        logging = ?{
                            log_level = #Debug;
                            is_running_locally = true;
                ...
                let zendb = ZenDB.launchDefaultDB(zendb_hstore);
                inclusivity_tests(zendb);
            },
        );
