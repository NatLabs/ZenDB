        ...
        suite(
            ...
            func() {
                ...
                for (record in inputs.vals()) {
                    let #ok(_) = crud_collection.insert(record) else return assert false;
                };
                ...
                for ((id, record) in crud_collection.entries()) {
                    ...
                    for (field in indexible_fields.vals()) {
                        ...
                        // let ?index = Map.get(stable_crud_collection.indexes, ZenDB.Types.thash, field # "_idx") else return assert false;

                        // let ?found_record = Itertools.find(
                        //     result.vals(),
                        //     func((search_id, _) : (Nat, RecordWithAllTypes)) : Bool {
                        //         Debug.print("Searching results for id " # debug_show id # " -> " # debug_show search_id);
                        //         search_id == id;
                        //     },
                        // ) else return assert false;
