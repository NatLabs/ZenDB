            ...
            func() {
                ...
                test(
                    ...
                    func() {
                        ...
                        let #ok(results) = users.search(db_query) else return assert false;
                        assert results.documents.size() == 0;
                ...
                test(
                    ...
                    func() {
                        ...
                        let #ok(update_result) = users.update(db_query, [("age", #Nat(0))]) else return assert false;
                ...
                test(
                    ...
                    func() {
                        ...
                        let before_count = before_results.documents.size();
                        ...
                        // Debug.print("deleted (" # debug_show (deleted.deleted_documents.size()) # ") " # debug_show (deleted));
                        for ((_, user) in deleted.deleted_documents.vals()) {
                            assert user.age == 0;
                        };

                        let #ok(after_results) = users.search(db_query) else return assert false;

                        // Debug.print("results after deletion (" # debug_show (after_results.documents.size()) # ") " # debug_show (after_results));

                        // Assert the right number were deleted
                        assert deleted.deleted_documents.size() == before_count;
                        assert after_results.documents.size() == 0;
                    },
                );
