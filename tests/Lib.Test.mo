...
suite(
    ...
    func() {
        ...
        test(
            ...
            func() {
                let scanned = HydraDB.scan<User>(hydra_db, "users", candify_user, [], []); // no start or terminate query
        ...
        let #ok = HydraDB.update<User>(hydra_db, "users", candify_user, _query, func(user : User) : User { { user with age = 0 } });
        ...
        let #ok(deleted_users) = HydraDB.delete<User>(
            hydra_db,
            "users",
            candify_user,
            _query,
