...
func getPaginatedResults(cursor: ?Nat, pageSize: Nat) : Result<([User], ?Nat), Text> {
  ...
  if (results.size() == pageSize) {
    ...
    return #ok(Array.map<(Nat, User), User>(results, func(entry) = entry.1), ?results[results.size()-1].0);
  } else {
    // No more results
    return #ok(Array.map<(Nat, User), User>(results, func(entry) = entry.1), null);
  }
}
```
...
ZenDB has been extensively benchmarked to compare its performance across different scenarios. Here are some key findings:

**Index Performance Improvement:**
- Full scan query on 10,000 records: ~226 million instructions
- Same query with proper indexes: ~2.4 million instructions (94x faster)

**Heap vs. Stable Memory:**
- Heap operations are generally 15-30% faster for read-heavy workloads
- Stable memory provides better space efficiency and persistence

**Multi-field Indexes:**
- Compound indexes optimize both filtering and sorting operations
- Provide 10-50x performance improvement for complex queries with both filtering and sorting

**Query Optimization Techniques:**
- Bitmap intersection for complex conditions shows 10-100x improvement over full scans
- Index-aware sorting eliminates the need for in-memory sort operations
- Multi-index coordination for complex queries with mixed AND/OR conditions
...
```motoko
let zendb_sstore = ZenDB.newStableStore(null);
```
...
```motoko
let db = ZenDB.launchDefaultDB(zendb_sstore);
```
...
```motoko
let #ok(cycles_db) = ZenDB.createDB(zendb_sstore, "cycles");
```

An existing database can also be retrieved by calling `getDB()` with the database's unique name:

```motoko
let #ok(cycles_db) = ZenDB.getDB(zendb_sstore, "cycles");
```
...
| #Tuple([SchemaType]) | A tuple with unnamed fields |
...
```motoko
let #ok(users_collection) = db.create_collection("unique_collection_name", UserSchema, candify);
```
...
```motoko
let #ok(_) = users_collection.create_index("name_idx", [("name", #Ascending)], false);
```
...
```motoko
let #ok(_) = users_collection.create_index("name_age_idx", [("name", #Ascending), ("age", #Ascending)], false);
```
